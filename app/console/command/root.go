...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&conf.Cmd.Host, "host", define.DefaultHost, "指定配置文件类型")
	RootCmd.PersistentFlags().StringVar(&conf.Cmd.Port, "port", define.DefaultPort, "指定优先级高于配置文件的日志存储位置：stderr|stdout|目录路径")
	...
	RootCmd.PersistentFlags().StringVar(&conf.Cmd.LogLevel, "log_level", define.DefaultLogLevel, "指定优先级高于配置文件的日志级别：debug|info|warn|error|panic|fatal")
	RootCmd.PersistentFlags().StringVar(&conf.Cmd.LogPath, "log_path", define.DefaultLogPath, "指定优先级高于配置文件的日志级别：debug|info|warn|error|panic|fatal")
}
